...
app.get('/', (request, response) => {
  loadPersons(()=>{
    loadRelations(()=>{
      const displayMessage = {text:`Persons loaded: ${personsData.length}, Relations loaded : ${relationsData.length}`, background:'blue'};
...
app.get('/personview/:id/:fname/:lname', (request, response)=>{
  ...
  if(!person){
    const displayMessage = {text:`${fname} ${lname} not found`, background:'red'};
    personsData.sort((a, b) => a.fname.localeCompare(b.fname));
...
app.post('/personadd', async (request, response)=>{
  ...
  if(rec.dob == ''){ rec.dob = null}; 
  if(rec.dod == ''){ rec.dod = null};   
  ...
  const person = personsData.find(item=>(item.fname).toLowerCase()===(rec.fname).toLowerCase() 
    && (item.lname).toLowerCase()===(rec.lname).toLowerCase());
  if (person){
    const displayMessage = {text:`${rec.fname} ${rec.lname} already exists`, background:'red'};
    personsData.sort((a, b) => a.fname.localeCompare(b.fname));
    response.render('persons', {displayMessage, personsData});
  }
  ...
  db.run(sql, [fname, lname, gender, isalive, dob, dod], (error)=>{
    if(error){
      const displayMessage = {text:`Database Error: Could not add ${rec.fname} ${rec.lname}`, background:'red'};
      personsData.sort((a, b) => a.fname.localeCompare(b.fname));
      return response.render('persons', {displayMessage, personsData});
    }
    loadPersons(()=>{
      const displayMessage = {text:`Added ${rec.fname} ${rec.lname} successfully`, background:'green'};
      personsData.sort((a, b) => a.fname.localeCompare(b.fname));
      response.render('persons', {displayMessage, personsData});
    });
  });
...
app.get('/persondelete/:id/:fname/:lname', (request, response)=>{
  ...
  if(relations.length){
    const displayMessage = {text:`Cannot delete ${fname} ${lname}. Relations already exists`, background:'red'};
    return response.render('persons', {displayMessage, personsData});
  }
    ...
    db.run(sql, id, (error)=>{
      if(error){
        const displayMessage = {text:`Database Error: Could not delete ${fname} ${lname}`, background:'red'};
        return response.render('persons', {displayMessage, personsData});
       }
       personsData = personsData.filter((item)=>{
        return item.id != id;
       });
       const displayMessage = {text:`${fname} ${lname} deleted`, background:'red'};
       return response.render('persons', {displayMessage, personsData});
   });
});
...
app.get('/persondelete/:id/:fname', (request, response)=>{
  ...
  if(relations.length){
    const displayMessage = {text:`Cannot delete ${fname}. Relations already exists`, background:'red'};
    return response.render('persons', {displayMessage, personsData});
  }
    const sql = `DELETE FROM persons
                  WHERE id = ?`;
    db.run(sql, id, (error)=>{
      if(error){
        const displayMessage = {text:`Database Error: Could not delete ${fname}`, background:'red'};
        return response.render('persons', {displayMessage, personsData});
       }
       personsData = personsData.filter((item)=>{
        return item.id != id;
...
app.post('/personedit', (request, response)=>{
  ...
  const person = personsData.find(item=>(item.fname).toLowerCase()===(rec.fname).toLowerCase() 
    && (item.lname).toLowerCase===(rec.lname).toLowerCase && item.id != id);
  if (person){
    const displayMessage = {text:`${rec.fname} ${rec.lname} already exists`, background:'red'};
    personsData.sort((a, b) => a.fname.localeCompare(b.fname));
    return response.render('persons', {displayMessage, personsData});
  ...
  db.run(sql, [fname, lname, gender, isalive, dob, dod, id], (error)=>{
    ...
    loadPersons(()=>{
      const displayMessage = {text:`Updated ${rec.fname} ${rec.lname} successfully`, background:'green'};
      personsData.sort((a, b) => a.fname.localeCompare(b.fname));
      response.render('persons', {displayMessage, personsData});
    });
  });
...
app.get('*', (request, response) => {
  const displayMessage = {text:`The page you are trying to reach in not available`, background:'red'};
  response.render('home', {displayMessage})
});
